...
describe Caffeinate::ActionMailer do
  class HelpersTestMailer < ::ActionMailer::Base
    def test(subscription)
      mail do |format|
        format.text { render plain: "#{Caffeinate::Engine.routes.url_helpers.unsubscribe_campaign_subscription_url(subscription.token, Rails.application.routes.default_url_options)}" }
      end
    end
  end
